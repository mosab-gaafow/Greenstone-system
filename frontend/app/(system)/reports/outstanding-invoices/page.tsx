    ...
    <div className="w-full space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <ReportExportBar source="reports/outstanding-invoices" params={{ from: range.from, to: range.to, search: search || undefined, paymentStatus: paymentStatus !== "All" ? paymentStatus : undefined }} fileName="Outstanding_Invoices" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <ReportKpiCard label="Outstanding inv." value={d?.summary.invoiceCount ?? 0} tone="blue" />
        <ReportKpiCard label="Total invoiced" value={`KES ${Number(d?.summary.totalInvoiced ?? 0).toLocaleString()}`} tone="blue" />
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : d?.rows && d.rows.length > 0 ? (
            <div className="overflow-x-auto"><table className="w-full text-sm">
              ...
              <tbody>{d.rows.map((inv) => (
                <tr key={inv.invoiceId} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="p-2.5 text-xs"><span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium ${inv.paymentStatus === 'Unpaid' ? 'bg-gray-100 text-gray-600' : 'bg-amber-50 text-amber-700'}`}>{inv.paymentStatus}</span></td>
                </tr>
              ))}</tbody>
