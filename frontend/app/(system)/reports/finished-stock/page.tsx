...
import { useFinishedStockReport } from '@/features/reports/hooks/use-reports';
...
export default function FinishedStockPage() {
  ...
  const q = useFinishedStockReport({ search: search || undefined });
  const d = q.data; const loading = q.isLoading || q.isFetching;

  return (
    <div className="w-full space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><Link href="/reports" className="text-xs text-muted-foreground hover:text-foreground inline-flex items-center gap-1 mb-1"><ArrowLeft className="size-3" />Back to Reports</Link><h1 className="text-xl font-bold">Finished Stock Report</h1><p className="text-xs text-muted-foreground">Current stock levels — physical, reserved, and available quantities.</p></div>
                  <ReportExportBar source="reports/finished-stock" params={{ search: search || undefined }} fileName="Finished_Stock" />
      ...
      <Card>
        <CardHeader><CardTitle className="text-sm">Stock by product</CardTitle></CardHeader>
        <CardContent className="p-0">
          ...
          ) : <p className="p-4 text-sm text-muted-foreground">No stock records found.</p>}
        </CardContent>
      </Card>
    </div>
  );
