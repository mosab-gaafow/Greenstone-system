...
export default function PurchasePaymentsReportPage() {
  ...
  const q = usePurchasePaymentsReport({ ...range, search: search || undefined, status: status !== 'All' ? status : undefined, paymentMethod: method !== 'All' ? method : undefined });
    ...
    <div className="w-full space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-bold">Purchase Payments Report</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="Search payment #, supplier, reference…" className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <select className="h-9 rounded-md border bg-card px-2 text-xs" value={status} onChange={(e) => setStatus(e.target.value)}>
          {PAYMENT_STATUSES.map(s => <option key={s} value={s}>{s === 'All' ? 'All statuses' : s}</option>)}
        </select>
      ...
      <Card>
        <CardHeader><CardTitle className="text-sm">Purchase payment records</CardTitle></CardHeader>
        <CardContent className="p-0">
          ...
          ) : <p className="p-4 text-sm text-muted-foreground">No purchase payments for this period and filters.</p>}
        </CardContent>
      </Card>
    </div>
  );
