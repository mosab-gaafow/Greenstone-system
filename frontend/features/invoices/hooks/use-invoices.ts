...
export function useInvoices(f: { page: number; pageSize: number; search?: string; orderId?: string; customerId?: string; status?: string; paymentStatus?: string }) {
  return useQuery({
    queryKey: invoiceKeys.list(f),
    queryFn: () => api.fetchInvoices(f),
