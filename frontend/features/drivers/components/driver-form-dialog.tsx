...
/** Add/Edit Driver — a centred Dialog on desktop, a full-screen Sheet below `sm`. */
export function DriverFormDialog({ open, onOpenChange, driver, pending, onSubmit }: DriverFormDialogProps) {
  const isDesktop = useMediaQuery('(min-width: 640px)');

  const title = driver ? `Edit ${driver.name}` : 'Add driver';
  const description = driver ? 'Update the driver details.' : 'Register a new driver.';
