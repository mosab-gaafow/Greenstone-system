...
export function useOrderSummary() {
  const [all, cash, credit] = useQueries({
    queries: [
      {
      ...
      {
        queryKey: orderKeys.list({ page: 1, pageSize: 1, paymentType: 'CASH' }),
        queryFn: () => ordersApi.fetchOrders({ page: 1, pageSize: 1, paymentType: 'CASH' }),
      },
      {
        queryKey: orderKeys.list({ page: 1, pageSize: 1, paymentType: 'CREDIT' }),
        queryFn: () => ordersApi.fetchOrders({ page: 1, pageSize: 1, paymentType: 'CREDIT' }),
      },
    ],
  });
  ...
  return {
    ...
    cash: cash.data?.meta.totalRecords,
    credit: credit.data?.meta.totalRecords,
    isLoading: all.isPending || cash.isPending || credit.isPending,
  };
}
